...
class LoginNotificationsPlugin extends Plugin
{
  ...
  public function __construct()
  {
    ...
    public function loginHandler(EventInterface $event)
    {
      ...
      //  $this->Date->setTimezone(Configure::get('Blesta.company_timezone'));
        ...
        if (count($user_logs) < 2) {
            ...
            // Check if the IP address matches the one from the 2nd item in the array
            if ($_SERVER['REMOTE_ADDR'] == $user_logs[1]->ip_address && $settings['send_on_new_ip'] == true) {
                // Set the return value for the event and exit the function
                return $event->setReturnValue($return);
        }
      }
    ...
    public function sendLoginNotificationEmail($type, $user, $settings, $geoData, $params)
    {
        ...
        if ($settings['enable_' . $type . '_login_emails'] == true) {
            // Define email content tags
            $tags = array(
                'ip_address' => $_SERVER['REMOTE_ADDR'],
                'user' => $user->username,
                'date' => date('D, d M Y', strtotime($this->Date->format('c'))),
                'time' => $this->Date->cast($this->Date->format('c'), 'g:i:s A'),
